...
function App() {
  ...
  const contactInfoPermanentRef = useRef(false); // Track if button should stay visible (using ref to avoid dependency issues)
  ...
  useEffect(() => {
    ...
    const handleVideo3FadeOut = () => {
      ...
      // Hide morphing text when video 3 ends, but keep contact info if it became permanent
      setShowMorphingText(false);
      if (!contactInfoPermanentRef.current) {
        setShowContactInfo(false);
        setBrandRevealDone(false);
        setTaglineRevealDone(false);
      }
      // Always reset these states for text animations
      setTextTypeFading(false);
      setBrandNameFading(false);
    };
  ...
  useEffect(() => {
    if (brandRevealDone && taglineRevealDone) {
      console.log("Both brand and tagline done, showing contact info");
      setShowContactInfo(true);
      contactInfoPermanentRef.current = true; // Make button permanently visible once it appears
    }
    ...
    <ResponsiveWrapper>
      ...
      <div className="App" style={{ position: "relative", zIndex: 10 }}>
        <div className="overlay-content">
          ...
          >
            {/* Text for Video 1 - only show when NOT in TextType mode */}
            {!showTextType && (
              <>
                <ScrambledText
                  className="big-text"
                ...
                <ScrambledText
                  ...
                  scrambleChars=":."
                >
                  Aevum Labs was founded to redefine the limits of healthy aging
                  and unlock longer, healthier lives for all.
                </ScrambledText>
              </>
            )}
                      ...
                      >
                        <TextType
                          ...
                          onSentenceComplete={(text, index) => {
                            ...
                            if (index === 2) {
                              // 1. Start the crossfade much earlier for longer morphing animation
                              setTimeout(() => {
                                // 2. Start the fade-out transition
                                setTextTypeFading(true);

                                // 3. After fade-out completes, show morphing text immediately
                                setTimeout(() => {
                    ...
                    {showMorphingText && (
                      <div
                        ...
                        style={{
                          ...
                          height: "100%",
                          textAlign: "center",
                          opacity: brandNameFading ? 0 : 0.95,
                      ...
                      >
                        <div className="w-full text-center brand-name">
                          <MorphingText
                            ...
                            onAnimationComplete={() => {
                              console.log("Brand animation completed");
                              setBrandRevealDone(true);
                            }}
                          />
                        </div>
                        ...
                        <div
                          className="w-full text-center tagline-text"
        ...
        {/* Contact information - shows when tagline completes and stays sticky thereafter */}
